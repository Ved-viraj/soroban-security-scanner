...
impl AppState {
    async fn new() -> Self {
        ...
        lockout_service
            ...
            .unwrap();
        lockout_service
            .add_config("admin", LockoutConfig::strict())
            .unwrap();
...
// Protected routes
async fn get_profile(State(_state): State<AppState>) -> Json<serde_json::Value> {
...
async fn rate_limited_endpoint(
    ...
    match state
        .auth_services
        .rate_limit_service
        .check_rate_limit(rate_limit_key, "strict")
        .await
    {
        Ok(result) => {
            if !result.allowed {
