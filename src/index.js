...
async function sendScanNotifications(service, contractPath, vulnerabilities, userEmail) {
  try {
    ...
    if (result.success) {
      ...
      console.log(
        chalk.yellow(
          t('commands.scan.some_notifications_failed', { channels: result.failedChannels })
        )
      );
    }
  } catch (error) {
...
program.name('soroban-security-scanner').description(t('scanner.description')).version('1.0.0');
    ...
    try {
      ...
      if (emergencyStop.isActive()) {
        ...
        if (vulnerabilities.length > 0) {
          console.log(
            chalk.yellow(t('commands.scan.partial_results', { count: vulnerabilities.length }))
          );
        }
        return;
      }
      ...
      .action(async options => {
        try {
          ...
          if (result.success) {
            ...
            console.log(
              chalk.blue(
                t('commands.notifications.test.delivered_to', {
            ...
            console.log(
              chalk.yellow(t('commands.notifications.test.failed_channels'), result.failedChannels)
            );
          }
          ...
          for (const [channel, channelStats] of Object.entries(stats)) {
            console.log(
              `  ${channel}: ${channelStats.totalSent} sent, ${channelStats.totalFailed} failed`
            );
          }
        } catch (error) {
          console.error(
            chalk.red(t('commands.notifications.test.test_failed', { error: error.message }))
          );
        }
      })
  )
      ...
      .action(async () => {
        try {
          ...
          for (const template of templates) {
            ...
            console.log(
              `     ${template.description || t('commands.notifications.templates.no_description')}`
            );
            console.log(
              `     ${t('commands.notifications.templates.channels', { channels: template.supportedChannels.join(', ') })}`
            );
            console.log(
              `     ${t('commands.notifications.templates.variables', { variables: template.variables.map(v => v.name).join(', ') })}`
            );
          }
        } catch (error) {
          console.error(
            chalk.red(t('commands.notifications.templates.list_failed', { error: error.message }))
          );
        }
      })
  )
  .addCommand(
    new Command('in-app').description('Manage in-app notifications').addCommand(
      new Command('list')
        ...
        .action(async userId => {
          try {
            ...
            console.log(
              chalk.blue(t('commands.notifications.in_app.list.notifications_for', { userId }))
            );
            ...
            console.error(
              chalk.red(
                t('commands.notifications.in_app.list.list_failed', { error: error.message })
              )
            );
          }
        })
