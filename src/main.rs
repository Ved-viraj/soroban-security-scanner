...
fn run_comprehensive_scan(
    ...
    for invariant_result in invariant_results {
        if let Some(security_result) = combined_results
            .iter_mut()
            .find(|r| r.file_path == invariant_result.file_path)
        {
            security_result
                .invariant_violations
                .extend(invariant_result.invariant_violations);
            security_result
                .recommendations
                .extend(invariant_result.recommendations);
        } else {
...
fn parse_report_format(format: &str) -> Result<ReportFormat> {
    match format.to_lowercase().as_str() {
        ...
        _ => anyhow::bail!(
            "Invalid output format: {}. Use: console, json, html, markdown",
            format
        ),
    }
}
...
fn run_k8s_scan(
    ...
    rt.block_on(async {
        ...
        let result = manager
            .execute_scan(&scan_id, &config, &contract_code)
            .await?;
        ...
        match action {
            ...
            K8sAction::Cleanup { age_minutes } => {
                ...
                let cleaned_count = manager
                    .cleanup_stuck_scans(Duration::from_secs(age_minutes * 60))
                    .await?;
    ...
    rt.block_on(async {
        match action {
            TimeTravelAction::Fork {
                ...
                println!(
                    "✅ Successfully forked at ledger {}",
                    forked_state.ledger_sequence()
                );
                println!(
                    "📅 Ledger timestamp: {}",
                    forked_state.ledger_snapshot.close_time
                );
                println!(
                    "⏱️  Fork created: {:.2}s ago",
                    forked_state.age().as_secs_f64()
                );
            }

            TimeTravelAction::Test {
                contract_id,
                ledger_sequence,
                rpc_url,
                format,
                ...
                println!(
                    "🧪 Testing contract {} against ledger {}",
            ...
            TimeTravelAction::Upgrade {
                ...
                println!(
                    "🔄 Simulating upgrade for contract {} at ledger {}",
                    contract_id, ledger_sequence
                );
                ...
                let upgrade_result = debugger
                    .simulate_contract_upgrade(&contract_id, &new_wasm, ledger_sequence)
                    .await?;

                println!("📊 Upgrade Simulation Results:");
                println!(
                    "  Compatible: {}",
                    if upgrade_result.is_compatible {
                        "✅ YES"
                    } else {
                        "❌ NO"
                    }
            ...
            TimeTravelAction::Orphaned {
                ...
                println!(
                    "🔍 Analyzing orphaned state for contract {} at ledger {}",
                    contract_id, ledger_sequence
                );
                ...
                let orphaned_entries = debugger
                    .get_orphaned_state(&contract_id, ledger_sequence, &new_wasm)
                    .await?;
...
fn run_batch_action(action: BatchAction) -> Result<()> {
    ...
    match action {
        BatchAction::CreateEscrowRelease {
            escrow_ids,
            requester,
        } => {
        ...
        BatchAction::ExecuteEscrowRelease { batch_id, executor } => {
            ...
            let summary =
                BatchOperations::execute_batch_escrow_release(env.clone(), batch_id, executor_addr);
        ...
        BatchAction::CreateVerification {
            vulnerability_ids,
            verifier,
        } => {
            ...
            let batch_id =
                BatchOperations::create_batch_verification(env.clone(), vuln_id_vec, verifier_addr);
        ...
        BatchAction::ExecuteVerification { batch_id, executor } => {
            ...
            let executor_addr = Address::from_string(&executor);
            let summary =
                BatchOperations::execute_batch_verification(env.clone(), batch_id, executor_addr);
        ...
        BatchAction::EstimateGas {
            ...
            if verbose {
                ...
                for item in &estimate.item_estimates {
                    println!(
                        "    Item {}: {:?} — {} gas",
                        item.item_index, item.operation_type, item.estimated_gas
                    );
                }
            }
            ...
            if estimate.exceeds_recommended_threshold {
                println!(
                    "  💡 Recommendation: Split into {} batches of ~{} items each",
...
fn run_differential_fuzzing_action(action: DifferentialFuzzingAction) -> Result<()> {
    ...
    match action {
        DifferentialFuzzingAction::Run { contract_path, .. } => {
            println!(
                "Running differential fuzzing on: {}",
        ...
        DifferentialFuzzingAction::TestWithNetworkState {
            ledger_sequence, ..
        } => {
        ...
        DifferentialFuzzingAction::CompareVersions {
            version1, version2, ..
        } => {
    ...
    match action {
        ...
        EmergencyStopAction::Test => {
            ...
            assert!(
                ...
                "Emergency stop should be inactive initially"
            );
            ...
            assert!(
                emergency_stop.is_stopped(),
                "Emergency stop should be active after trigger"
            );
