...
impl SecurityScanner {
    ...
    fn initialize_patterns(&mut self) -> Result<()> {
        ...
        self.add_pattern(
            VulnerabilityType::EventLoggingBypass,
    ...
    pub fn scan_file(&self, file_path: &Path) -> Result<ScanResult> {
        ...
        self.watchdog
            .heartbeat_with_file(&file_path.to_string_lossy());
    ...
    fn is_vulnerability_context_valid(
        &self,
        _syntax: &syn::File,
        content: &str,
...
impl InvariantScanner {
    ...
    fn initialize_rules(&mut self) -> Result<()> {
        ...
        self.add_rule(
            InvariantRule::BalanceIntegrity,
    ...
    pub fn scan_directory(&self, dir_path: &Path) -> Result<Vec<ScanResult>> {
        ...
        for entry in walkdir::WalkDir::new(dir_path) {
            ...
            if self.emergency_stop.is_stopped() || self.watchdog.has_timed_out() {
                info!(
                    "Invariant directory scan cancelled due to emergency stop or watchdog timeout"
                );
                break;
            }
